...
public class RatingManagement_Second {
    private ArrayList<Rating> ratings;
    private ArrayList<Movie> movies;
    private ArrayList<User> users;

    // @Requirement 1
    public RatingManagement_Second(String moviePath, String ratingPath, String userPath) {
    ...
    public ArrayList<Movie> findMoviesByNameAndMatchRating(int userId, int rating) {
        ...
        ArrayList<Rating> ratingOfUserIdMatchRequire = new ArrayList<>();

        for (Rating r : ratings) {
            if (r.getUserId() == userId && r.getRating() >= rating) {
               ratingOfUserIdMatchRequire.add(r);
            }
        }

        for (Rating r : ratingOfUserIdMatchRequire) {
            for (Movie m : movies) { 
